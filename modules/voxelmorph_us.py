...
class U_Network(nn.Module):
    ...
    def forward(self, src, tgt):
        x = torch.cat([src, tgt], dim=1)#图像堆叠
        # Get encoder activations
        x_enc = [x]
        for i, l in enumerate(self.enc):#每个卷积结果都保存
            x = l(x_enc[-1])
            x_enc.append(x)
...
class SpatialTransformer(nn.Module):
    def __init__(self, size, mode='bilinear'):
        ...
        vectors = [torch.arange(0, s) for s in size]#两个0-224列表
        grids = torch.meshgrid(vectors)#生成格网
